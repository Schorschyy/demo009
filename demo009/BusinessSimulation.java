...
public class BusinessSimulation {
	...
	public void play() {
		...
		while(true) {
			...
			System.out.println("========== Niederlassungen \nOrt \t\tWarenart \t Prod. angefordert \t ges. Furhwerke \t Arbeiter ");
			
			for(int z = 0; z <= ApplicationContext.niederlassungen.size(); z++){
				DisplayService.anzeigenNiederlassung(ApplicationContext.niederlassungen.get(z));
		//	for (Niederlassung niederlassung : appContext.getNiederlassungen()) { 
		//	DisplayService.anzeigenNiederlassung(niederlassung.getOrt(),niederlassung.getWarenartString(),Boolean.toString(niederlassung.wurdeAngefordert()), 1,niederlassung.getAnzahlArbeitende());
