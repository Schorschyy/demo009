...
public class CommandVerkaufen implements Command {
	...
	public void execute(ApplicationContext context) {
		...
		System.out.println("========== Welche der verf�gbaren Warenarten soll verkauft werden?");
		...
		System.out.println(indexWarenart + ": Zur�ck");
		...
		while(!scannerErfolg) {
			try {
				...
				if(warenartMenge<=context.getLager().getBestand(ausgewaehlteWarenart)) {
					...
					System.out.println("Es k�nnen nur maximal "+context.getLager().getBestand(ausgewaehlteWarenart)+" Einheiten "+Util.convertWarenartToString(ausgewaehlteWarenart)+" verkauft werden.");
					scannerErfolg=false;
				}
				myScanner.nextLine();
			} catch (Exception e) {
